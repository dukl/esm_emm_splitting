...
int esm_proc_esm_information_response (emm_context_t * ue_context, pti_t pti, const_bstring const apn, const protocol_configuration_options_t * const pco, esm_cause_t * const esm_cause)
{
   ...
   */
  printf("----------------------------------------------------in esm_information.c---------------------------------------------------------\n\n");
  printf("start modify\n");
  printf("mcc:%d%d%d\n",ue_context->_guti.gummei.plmn.mcc_digit1,ue_context->_guti.gummei.plmn.mcc_digit2,ue_context->_guti.gummei.plmn.mcc_digit3);
  printf("mnc:%d%d%d\n",ue_context->_guti.gummei.plmn.mnc_digit1,ue_context->_guti.gummei.plmn.mnc_digit2,ue_context->_guti.gummei.plmn.mnc_digit3);
  printf("%d\n",ue_context->_guti.gummei.mme_gid);
  printf("%d\n",ue_context->_guti.gummei.mme_code);
  printf("%d\n",ue_context->_guti.m_tmsi);
  printf("end modify\n");
  printf("----------------------------------------------------in esm_information.c---------------------------------------------------------\n\n");

 // obj_hashtable_get(mme_app_desc.mme_ue_contexts.guti_ue_context_htbl,&ue_context->_guti,sizeof(guti_t),EsmContext);
  nas_stop_T3489(EsmContext[0]);
 // nas_stop_T3489(&ue_context->esm_ctx);

  if (apn) {
   // if (ue_context->esm_ctx.esm_proc_data->apn) {
  if((*EsmContext[0]).esm_proc_data->apn){
  //    bdestroy_wrapper(&ue_context->esm_ctx.esm_proc_data->apn);
    bdestroy_wrapper(&(*EsmContext[0]).esm_proc_data->apn);
    }
  //  ue_context->esm_ctx.esm_proc_data->apn = bstrcpy(apn);
  (*EsmContext[0]).esm_proc_data->apn = bstrcpy(apn);
  }

  if ((pco) && (pco->num_protocol_or_container_id)) {
  //  if (ue_context->esm_ctx.esm_proc_data->pco.num_protocol_or_container_id) {
  if ((*EsmContext[0]).esm_proc_data->pco.num_protocol_or_container_id){
  //    clear_protocol_configuration_options(&ue_context->esm_ctx.esm_proc_data->pco);
    clear_protocol_configuration_options(&(*EsmContext[0]).esm_proc_data->pco);
    }
  //  copy_protocol_configuration_options(&ue_context->esm_ctx.esm_proc_data->pco, pco);
  copy_protocol_configuration_options(&(*EsmContext[0]).esm_proc_data->pco,pco);
  }
 // obj_hashtable_insert(mme_app_desc.mme_ue_contexts.guti_ue_context_htbl,&ue_context->_guti,sizeof(guti_t),EsmContext);
...
static void _esm_information_t3489_handler (void *args)
{
  ...
  if (esm_ebr_timer_data) {
    ...
    if (esm_ebr_timer_data->count < ESM_INFORMATION_COUNTER_MAX) {
       ...
       * Stop timer T3489
       */
	//obj_hashtable_get(mme_app_desc.mme_ue_contexts.guti_ue_context_htbl,&esm_ebr_timer_data->ctx->_guti,sizeof(guti_t),EsmContext);
	//(*EsmContext[0]).T3489.id = NAS_TIMER_INACTIVE_ID;
	//obj_hashtable_insert(mme_app_desc.mme_ue_contexts.guti_ue_context_htbl,&esm_ebr_timer_data->ctx->_guti,sizeof(guti_t),EsmContext);
...
//  (*EsmContext[0]).T3489.id = nas_timer_start ((*EsmContext[0]).T3489.sec, 0 /*usec*/,_esm_information_t3489_handler, data);
...
//    OAILOG_INFO (LOG_NAS_EMM, "UE " MME_UE_S1AP_ID_FMT "Timer T3489 (%lx) expires in %ld seconds\n",
//        ue_id, (*EsmContext[0]).T3489.id, (*EsmContext[0]).T3489.sec);
  //obj_hashtable_insert(mme_app_desc.mme_ue_contexts.guti_ue_context_htbl,&ue_context->_guti,sizeof(guti_t),EsmContext);
