  ...
  const bool is_emergency)
{
  ...
  if (!ue_mm_context->pdn_contexts[pdn_cid]) {
	...
	printf("---------------------------------------------In PdnConnectivity.c------------------------------------------------------\n\n");
	printf("modify_start\n");
	printf("mcc:%d%d%d\n",ue_mm_context->emm_context._guti.gummei.plmn.mcc_digit1,ue_mm_context->emm_context._guti.gummei.plmn.mcc_digit2,ue_mm_context->emm_context._guti.gummei.plmn.mcc_digit3);
	printf("mnc:%d%d%d\n",ue_mm_context->emm_context._guti.gummei.plmn.mnc_digit1,ue_mm_context->emm_context._guti.gummei.plmn.mnc_digit2,ue_mm_context->emm_context._guti.gummei.plmn.mnc_digit3);
	printf("%d\n",ue_mm_context->emm_context._guti.gummei.mme_gid);
	printf("%d\n",ue_mm_context->emm_context._guti.gummei.mme_code);
	printf("%ld\n",ue_mm_context->emm_context._guti.m_tmsi);

	/*
    hashtable_rc_t getState = obj_hashtable_get(mme_app_desc.mme_ue_contexts.guti_esm_context_htbl,&ue_mm_context->emm_context._guti,sizeof(guti_t),EsmContext);
	switch(getState){
		case HASH_TABLE_BAD_PARAMETER_HASHTABLE:
			printf("hashtblp == NULL \n");break;
	  ...
	  printf("1\n");
	  printf("%d\n",(*EsmContext[0]).n_pdns);
	  (*EsmContext[0]).n_pdns += 1;
	  printf("2\n");
	  obj_hashtable_insert(mme_app_desc.mme_ue_contexts.guti_esm_context_htbl,&ue_mm_context->emm_context._guti,sizeof(guti_t),EsmContext);
	  */
	  printf("modify_end\n");
	printf("---------------------------------------------In PdnConnectivity.c------------------------------------------------------\n\n");
...
proc_tid_t _pdn_connectivity_delete (emm_context_t * emm_context, pdn_cid_t pdn_cid)
{
  ...
  if (pti != ESM_PT_UNASSIGNED) {
    ...
    ue_mm_context->emm_context.esm_ctx.n_pdns -= 1;
//	printf("---------------------------------------------In PdnConnectivity.c------------------------------------------------------\n\n");
//	obj_hashtable_get(mme_app_desc.mme_ue_contexts.guti_esm_context_htbl,&ue_mm_context->emm_context._guti,sizeof(guti_t),EsmContext);
//	(*EsmContext[0]).n_pdns -= 1;
//	obj_hashtable_insert(mme_app_desc.mme_ue_contexts.guti_esm_context_htbl,&ue_mm_context->emm_context._guti,sizeof(guti_t),EsmContext);
//	printf("\ndukl\n");
//	printf("---------------------------------------------In PdnConnectivity.c------------------------------------------------------\n\n");
