...
esm_context_t * EsmContext[1];
  ...
  printf("--------------------------------------------------in esm_recv.c_start-------------------------------------\n\n");
  obj_hashtable_get(mme_app_desc.mme_ue_contexts.guti_ue_context_htbl,&emm_context->_guti,sizeof(guti_t),EsmContext);
  printf("mcc:%d%d%d\n",emm_context->_guti.gummei.plmn.mcc_digit3,emm_context->_guti.gummei.plmn.mcc_digit2,emm_context->_guti.gummei.plmn.mcc_digit1);
  printf("mnc:%d%d%d\n",emm_context->_guti.gummei.plmn.mnc_digit3,emm_context->_guti.gummei.plmn.mnc_digit2,emm_context->_guti.gummei.plmn.mnc_digit1);
...
if(EsmContext == NULL){
	printf("EsmContext==NULL\n");
}else if((EsmContext[0])==NULL){
	printf("EsmContext[0]==NULL\n");
}//else if((*EsmContext[0]).esm_proc_data == NULL)
 //   printf("wrong\n");
//printf("%d\n",(*EsmContext[0]).T3489.id);
//if (!(*EsmContext[0]).esm_proc_data) {
  printf("--------------------------------------------------in esm_recv.c_end-------------------------------------\n\n");

    emm_context->esm_ctx.esm_proc_data  = (esm_proc_data_t *) calloc(1, sizeof(*emm_context->esm_ctx.esm_proc_data));
//  (*EsmContext[0]).esm_ctx_proc_data = (esm_proc_data_t *) calloc(1,sizeof(*(*EsmContext[0]).esm_proc_data));
  }

  struct esm_proc_data_s * esm_data = emm_context->esm_ctx.esm_proc_data;
//struct esm_proc_data_s * esm_data = (*EsmContext[0]).esm_proc_data;
  ...
  esm_cause = ESM_CAUSE_SUCCESS;
#endif
//obj_hashtable_insert(mme_app_desc.mme_ue_contexts.guti_ue_context_htbl,&emm_context->_guti,sizeof(guti_t),EsmContext);
  /*
   * Return the ESM cause value
   */
  OAILOG_FUNC_RETURN (LOG_NAS_ESM, esm_cause);
...
//  nas_itti_pdn_config_req(pti,ue_id,&emm_context->_imsi,(*EsmContext[0]).esm_proc_data,(*EsmContext[0]).esm_proc_data->request_type);
