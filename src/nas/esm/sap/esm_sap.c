		...
		printf("Already get a message from TASK ESM SAP !!--wluhan\n");                 
		 ...
		 esm_sap_test_t * msg=&(ESM_DATA_IND(received_message_p));
 		 ...
 		 OAILOG_INFO (LOG_NAS_ESM, "ESM-SAP   - Received primitive %s (%d)\n", _esm_sap_primitive_str[primitive - ESM_START - 1], primitive);
		 	...
		 	printf("Flags_dukl!!!\n\n");
			printf("---------------------------------------------in eam_sap.c---------------------------------------------\n\n");
			printf("mcc:%d%d%d\n",msg->ctx->_guti.gummei.plmn.mcc_digit3,msg->ctx->_guti.gummei.plmn.mcc_digit2,msg->ctx->_guti.gummei.plmn.mcc_digit1);
			printf("mnc:%d%d%d\n",msg->ctx->_guti.gummei.plmn.mnc_digit3,msg->ctx->_guti.gummei.plmn.mnc_digit2,msg->ctx->_guti.gummei.plmn.mnc_digit1);
            printf("%d\n",msg->ctx->_guti.gummei.mme_gid);
            printf("%d\n",msg->ctx->_guti.gummei.mme_code);
            printf("%ld\n",msg->ctx->_guti.m_tmsi);
			printf("---------------------------------------------in esm_sap.c---------------------------------------------\n\n");
		  ...
		  case ESM_PDN_CONNECTIVITY_REQ:
		     ...
		     */
		 	printf("ESM_PDN_CONNECTIVITY_REQ\n");
		  ...
		  case ESM_PDN_CONNECTIVITY_REJ:
		 	printf("ESM_PDN_CONNECTIVITY_REJ\n");
		  ...
		  case ESM_DEFAULT_EPS_BEARER_CONTEXT_ACTIVATE_CNF:
		 	printf("ESM_DEFAULT_EPS_BEARER_CONTEXT_ACTIVATE_CNF\n");
		  ...
		  case ESM_DEFAULT_EPS_BEARER_CONTEXT_ACTIVATE_REJ:
		 	printf("ESM_DEFAULT_EPS_BEARER_CONTEXT_ACTIVATE_REJ\n");
		  ...
		  case ESM_DEDICATED_EPS_BEARER_CONTEXT_ACTIVATE_REQ: {
		 	printf("ESM_DEFAULT_EPS_BEARER_CONTEXT_ACTIVATE_REQ\n");
		  ...
		  case ESM_EPS_BEARER_CONTEXT_DEACTIVATE_REQ:{
		 	printf("ESM_EPS_BEARER_CONTEXT_DEACTIVATE_REQ\n");
  ...
  esm_sap_error_t * err)
{
  ...
  printf("esm_msg.header.message_type = %d\n",esm_msg.header.message_type);
  /*
