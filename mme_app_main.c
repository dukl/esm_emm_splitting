...
int mme_app_init (const mme_config_t * mme_config_p)
{
  ...
  btrunc(b, 0);
  bassigncstr(b, "mme_app_guti_esm_context_htbl");
  mme_app_desc.mme_ue_contexts.guti_esm_context_htbl = obj_hashtable_create(mme_config.max_ues,NULL,NULL,NULL,b);
  bdestroy_wrapper (&b);
...
void mme_app_exit (void)
{
  ...
  obj_hashtable_destroy(mme_app_desc.mme_ue_contexts.guti_esm_context_htbl);
  mme_config_exit();
}
